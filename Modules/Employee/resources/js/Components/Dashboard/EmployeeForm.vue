                        ...
                        <div class="space-y-3">
                            <FormItemCard v-for="(item, idx) in form.job_experiences" :key="item._key" :title="__('Experience')" :index="idx" @remove="removeJobExperience(idx)">
                                <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                                    ...
                                    <div class="space-y-2"><Label class="text-xs font-medium">{{ __('Province') }}</Label><Input v-model="item.province" :placeholder="__('Province')" class="bg-background" /></div>
                                    <div class="space-y-2"><Label class="text-xs font-medium">{{ __('City') }}</Label><Input v-model="item.city" :placeholder="__('City')" class="bg-background" /></div>
                                    <div class="space-y-2"><Label class="text-xs font-medium">{{ __('Start Date') }}</Label><Input v-model="item.start_date" type="date" class="bg-background" /></div>
